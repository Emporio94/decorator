...
public class JavaFile {
    ...
    public Method getMethodByName(String methodName){
        Method firstMethod = methods.stream().filter(method -> method.getName().equals(methodName)).findFirst().orElse(null);
        if (firstMethod.getInputParameterType().size() == 0){
            Method secondMethod = methods.stream().filter(method -> method.getName().equals(methodName)).limit(2).skip(1).findFirst().orElse(null);
            if (secondMethod != null && secondMethod.getInputParameterType().size() > 0){
